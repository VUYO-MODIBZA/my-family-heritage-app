...
const NikiweChildren = () => {
  const [children, setChildren] = useState([]);

  useEffect(() => {
    const fetchNikiweChildrenData = async () => {
      try {
        const response = await fetch('http://localhost:5050/api/family/nikiwe_children');

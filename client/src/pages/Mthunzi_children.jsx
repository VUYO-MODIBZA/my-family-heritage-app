...
const MthunziChildren = forwardRef((props,ref) => {
  const [children, setChildren] = useState([]);

  useEffect(() => {
    const fetchMthunziChildrenData = async () => {
      try {
        const response = await fetch('http://localhost:5050/api/family/mthunzi_children');
        if (!response.ok) { 
...
export default MthunziChildren;
...
const NokhayaChildren = forwardRef((props,ref) => {
  const [children, setChildren] = useState([]);

  useEffect(() => {
    const fetchNokhayaChildrenData = async () => {
      try {
        const response = await fetch('http://localhost:5050/api/family/nokhaya_children');
        if (!response.ok) { 
      ...
      <div className="cards-container">
        <div className="card">
          <img
            ...
            alt="Tozi"/>
          <h2 className="card-title">Nokhaya</h2>
          <p className="card-text">Gender: Female</p>
          <p className="card-text">Deceased: "Yes"</p>
        </div>
      </div>
...
export default NokhayaChildren;

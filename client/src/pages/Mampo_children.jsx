...
const MampoChildren = forwardRef((props,ref) => {
  const [children, setChildren] = useState([]);

  useEffect(() => {
    const fetchMampoChildrenData = async () => {
      try {
        const response = await fetch('http://localhost:5050/api/family/mampo_children');
        if (!response.ok) { 
        ...
        console.error("Error fetching mampo_children data:", error);
      }
    };

    fetchMampoChildrenData();
  }, []);

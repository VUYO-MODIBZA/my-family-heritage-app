...
const SilasChildren = forwardRef((props,ref) => {
  const [children, setChildren] = useState([]);

  useEffect(() => {
    const fetchSilasChildrenData = async () => {
      try {
        const response = await fetch('http://localhost:5050/api/family/silas_children')
        if (!response.ok) { 
